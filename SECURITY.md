...
| `Sites.Read.All` | The harvest site, read through Graph: the site by URL, the device folders and files under `Harvest/`, and a file's short-lived download URL, for T01's **📁 From the harvest site** on Evidence (build 10617). Delegated — what the signed-in admin can open in SharePoint anyway; no admin consent required. Its own scope on purpose: a read is never bought with the `Sites.FullControl.All` write scope that happens to be here. TUNO never reads a file's bytes itself: the download URL is handed to the browser as a link (build 10621), and every call TUNO makes still goes to `graph.microsoft.com` and nowhere else |
