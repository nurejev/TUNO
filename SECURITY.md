...
| `Sites.Read.All` | The harvest site, read: the device folders under `Harvest/` and one bundle at a time, for T01's **📁 From the harvest site** on Evidence (build 10617). Delegated — what the signed-in admin can open in SharePoint anyway; no admin consent required. Its own scope on purpose: a read is never bought with the `Sites.FullControl.All` write scope that happens to be here |
