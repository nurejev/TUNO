...
|---|---|---|
| `Invoke-TunoAppLockerScan.ps1` | Scans a device and builds a rule set from what it finds. Since 1.13.0 it is also the remediation half of the scan-on-schedule pair: run as SYSTEM with no `-OutputPath` it switches to **Remediation mode** (output under `%ProgramData%\IT-TOOLS\LOGS\AppLockerScan`, console transcript next to the bundle, own output older than 30 days removed first, one summary line) and, with a harvest target (stamped in by T01, `HKLM\SOFTWARE\TUNO\Harvest`, or `-Harvest*` parameters), uploads the bundle to the harvest site as `Harvest/<device>/` (1.13.1: a bundle over 4 MB lands — the chunked upload's last part was mistyped). 1.14.0: a `.harvest-pending` / `.harvest-done` marker beside each bundle; a Remediation run uploads pending bundles first and, with the newest bundle inside the 7-day window, stops there — no rescan, exit 1 with the reason when an upload still fails | Upload the `.json` bundle to T01 — or download it from the harvest site with **📁 From the harvest site** on Evidence and upload it from there |
| `Detect-TunoAppLockerScan.ps1` | Detection half of the scan-on-schedule pair: exits 1 when the device has no `TunoAppLockerScan-*.json` younger than 7 days under `IT-TOOLS\LOGS\AppLockerScan` — and (1.1.0) when a bundle of the last 30 days did not reach the harvest site (pending marker, or a pre-1.14.0 transcript saying `harvest: upload FAILED`), reason on the console line, so the remediation uploads it without rescanning | Reference ring only; "Issue fixed" means "the scan ran" |
...
| `Get-TunoHarvestStatus.ps1` | Live Response answer to "the device folder on the harvest site is empty": every harvest target on the device (IME's cached copies of the deployed pairs, the registry override; secret masked), the uploader certificate, local bundles and sizes, the harvest lines of the last scan transcript and collector log, a note on a deployed copy older than 1.13.1 / 1.3.1; `-Probe` gets a token and resolves the site as the pairs do; `-ProbeUpload` writes one probe file | Console + `%ProgramData%\IT-TOOLS\LOGS\HarvestStatus_<host>_<stamp>.log`; `run Get-TunoHarvestStatus.ps1 -parameters "-Probe"` |
| `New-TunoHarvestUploaderApp.ps1` | Run once by an administrator: the **TUNO Harvest Uploader** app registration (`Sites.Selected`, application permission), a certificate (PFX + CER), and `write` on the harvest site T01 created — and nothing else | Prints the client id + certificate subject for T01's 📁 panel, a `reg add` block for hand-deployed collectors, and the Intune PKCS-import steps; needs the Microsoft.Graph module |
...
### The harvest site (build 10613)

Every retrieval above needs the device on. A **harvest site** is one SharePoint site the
collector uploads to on every pass — `Harvest/<COMPUTERNAME>/AppControlEvents_Bundle_<stamp>.json`
and `..._Report_<stamp>.html` — so the evidence is in the tenant whether the laptop is in
the bag or not. Three parts, in order:
...
2. **The uploader app** — from the 📁 panel itself (build 10615: creates the registration with `Sites.Selected`, consents it, adds a **client secret** shown once, and grants `write` on the site — three broad delegated writes on TUNO's registration, see SECURITY.md; the secret then travels in the collector's script body on every device, which the panel says before the button) — or **`New-TunoHarvestUploaderApp.ps1 -SiteUrl <the site>`**, run once by an administrator,
   ...
   that site only. It prints the client id and certificate subject, writes the PFX, and
   shows the Intune **PKCS imported certificate** steps for getting the PFX into every
   device's `LocalMachine\My` store with a non-exportable private key.
3. Paste the two values into the 📁 panel; the events Remediation created from the 🚀 panel
   from then on **carries the target** in its `HARVEST TARGET` block. A collector deployed
   by hand reads `HKLM\SOFTWARE\TUNO\Harvest` instead; a shell run takes `-Harvest*`.

On the device the collector signs a JWT with the certificate's private key (RS256,
`GetRSAPrivateKey`, so CAPI and CNG keys both work), exchanges it for an app-only Graph
...
| `-RetentionDays` | `30` as a Remediation, `0` interactively | Remove this script's own output in the output folder older than N days before scanning — earlier bundles, the two XML files, transcripts, by name. `0` keeps all. |
| `-HarvestSiteUrl` `-HarvestTenantId` `-HarvestClientId` `-HarvestCertSubject` / `-HarvestCertThumbprint` / `-HarvestClientSecret` | — | The harvest target (1.13.0), same three sources and precedence as the events collector: parameters › `HKLM\SOFTWARE\TUNO\Harvest` › the embedded HARVEST TARGET block T01 fills in. All-or-nothing; a partial target warns and uploads nothing. |

### As an Intune Remediation (1.13.0)

T01's 🚀 panel creates the pair (`Detect-TunoAppLockerScan.ps1` + this script), stamping
the harvest target into the HARVEST TARGET block when the 📁 panel has one. Run by the
Intune Management Extension — SYSTEM, no parameters — the scanner notices it and:

- writes the bundle to `%ProgramData%\IT-TOOLS\LOGS\AppLockerScan` and transcribes the
  console to `AppLockerScan-<stamp>.log` next to it (Intune keeps 2 KB of output; a scan
  says more than that);
- removes its own output there older than 30 days first (`-RetentionDays`);
- uploads the bundle to `<site>/Harvest/<COMPUTERNAME>/` when a target is configured —
  certificate in `LocalMachine\My` or client secret, exactly as `Get-TunoAppControlEvents.ps1`;
  an upload failure is a warning, the local bundle stays, the next pass retries;
- ends with one summary line and exit 0 (1 on a fatal error).

The detection half exits 1 when the newest bundle is older than 7 days (or there is
