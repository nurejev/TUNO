...
//      fetched at boot, so EVERY visitor gets the branding. The gear's
...
// to redress tuno.limon-it.nl from a browser. For the same reason
// BRANDING.host is NOT configurable here: it drives isProduction, the BETA
...
(() => {
  "use strict";
  const STORE = "tuno-selfhost-brand";
  const KEY = "selfhost";
  ...
  const rebrand = () => document.dispatchEvent(new CustomEvent("tuno:brand-updated"));
  ...
  function buildModal(prefill) {
    ...
    bg.innerHTML = `<div class="modal" style="max-width:680px">
      ...
      <p class="mini" style="margin:8px 0 0">Full per-theme palettes (every CSS variable, not just the identity five) fit the same file — edit the downloaded JSON's colorsLight / colorsDark by hand (css/app.css documents the variables), then ⭱ Import it here; the extra entries survive the round-trip.</p>
    ...
    $("shDownload").addEventListener("click", () => {
      const data = JSON.stringify({ v: 1, comment: "Serve this file as /selfhost-branding.json next to index.html.", brand: collect() || {} }, null, 2);
