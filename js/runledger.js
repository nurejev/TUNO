...
//
// Ported whole from ENCA (its build 25301), the house rule for
// ENCA-derived machinery: the same file, not a second implementation.
// TUNO's batch writers — T11's apply, T04's restore and assignment
// import, T25's disable/delete, T22's archived cleanup, the baseline
// acts — used to say what they were doing with a one-line prog() and
// print the table afterwards; the ledger shows the whole list BEFORE
// the first write and the outcome of each row as it lands.
//
...
const RunLedger = (() => {
  ...
  function create(host, opts) {
    const o = opts || {};
    // A plain string is a label and nothing else. (`"x".sub` is the legacy
    // String.prototype.sub — read as a property it printed "function sub()
    // { [native code] }" beside every string item; found by the TUNO port's
    // suite, reported back to ENCA.)
    const items = (o.items || []).map((it) => ({ label: typeof it === "string" ? it : it.label, sub: (it && typeof it === "object" && it.sub) || "", state: "pend", note: "", st: "" }));
    const t0 = Date.now();
