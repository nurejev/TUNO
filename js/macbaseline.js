...
const MacBaseline = (() => {
  ...
  const releaseOf = (name) => { const m = /\bR26\.(\d{1,3})\b/i.exec(name || ""); return m ? +m[1] : null; };
  ...
  const keyOf = (name) => String(name || "")
    .replace(/-?\s*\bR26\.\d{1,3}\b\s*/gi, " ")
    .replace(/-?\s*v\d+(?:\.\d+)*\s*$/i, " ")
    .replace(/[-_]/g, " ")
    .replace(/\s+/g, " ")
    .trim().toLowerCase();
  const relLabel = (rel) => rel == null ? "—" : `R26.${rel}`;
  ...
  function cmpRelVer(aRel, aVer, bRel, bVer) {
    if (aRel != null && bRel != null && aRel !== bRel) return aRel < bRel ? -1 : 1;
  ...
  function compare(vms, cat) {
    ...
    for (const b of cat.policies) {
      ...
      const bRel = b.release != null ? b.release : releaseOf(b.name);
  ...
  return {
    releaseOf, versionOf, looksBaseline, keyOf, relLabel, cmpVersion, cmpRelVer,
