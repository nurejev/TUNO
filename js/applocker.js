...
const AppLockerTool = (() => {
    ...
    if (scan) {
      ...
      rows.push(`<tr><td>🛰 Scan bundle <span class="tag grant">loaded</span></td><td class="mini">${esc(m.name || "device")} · scanner ${esc((scan.generator || {}).version || "?")}${m.elevated === false ? " · <b>NOT elevated</b>" : ""}</td><td class="mini">${esc(when)}</td>
        <td class="mini">${scan.writablePaths.length} writable dir${scan.writablePaths.length === 1 ? "" : "s"} · ${scan.writableFilesChecked ? scan.writableFiles.length + " writable file" + (scan.writableFiles.length === 1 ? "" : "s") : "files not checked"} · ${scan.artifacts.length} executables${pd ? "" : ` · <b>ProgramData not scanned</b> — Intune-deployed scripts outside IT-TOOLS were not seen`}${(() => {
          ...
          const all = Array.isArray(src.mdm) ? src.mdm : [];
          // A grouping with NO collections was never merged (scanner 1.12.0/1.12.1
          // walked the MDM cache three levels deep and reported the CSP area GUID
          // as a grouping with types: []) — say "nothing read", never "includes".
          const mdm = all.filter((g) => Array.isArray(g.types) && g.types.length);
          const hollow = all.length - mdm.length;
          if (mdm.length) return ` · effective policy includes Intune grouping${mdm.length === 1 ? "" : "s"} ${mdm.map((g) => `${esc(g.grouping)} (${esc(g.types.join(", "))})`).join(", ")}`;
          if (hollow) return ` · <b style="color:var(--off)">Intune AppLocker cache present on the device but NO collection was read from it</b> — scanner ${esc((scan.generator && scan.generator.version) || "?")} walked it too shallow; re-scan with 1.12.2 or later, this effective policy is missing the Intune part`;
  ...
  const REMEDY_PAIRS = {
    ...
    // 10627 (all device folders on the harvest site empty while the detection
    // said "nothing to do"): a bundle that did not reach the harvest site is
    // not done. The detection also exits 1 on a pending upload, and the
    // remediation then uploads it without rescanning.
    scan: {
