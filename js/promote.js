...
const PROMOTE = {
  ...
  items: [
    {
      n: 57,
      title: "There is always a sign-in \u2014 ENCA's model restored",
      tools: ["TUNO"],
      builds: [10376],
      risk: "high",
      what: "The agreed rule, made true again: every entry to the app is a real interactive sign-in. authInitInner() no longer restores cached accounts \u2014 handleRedirectPromise() completing is the ONE path that enters directly, because it is a sign-in finishing; an F5 or a reopened tab lands on the sign-in screen and the click runs loginPopup/loginRedirect with prompt:'select_account', ENCA's verbatim call, so the chooser appears and the IdP re-runs its policy, MFA included. The 10371 short-circuit (adopted session entering with no prompt), the getAllAccounts() restore and the #loginResume note are all removed \u2014 markup included. adopt() stays, for what it was always needed for: silent incremental-consent tokens WITHIN the session. Supersedes item 53 (number retired) and the restore/prompt halves of item 45 (rewritten to what survives).",
      why: "HIGH \u2014 the sign-in path for every tool, again, and this time in the strict direction: the accepted cost is an interactive prompt (and the tenant's MFA policy) on every entry, which is ENCA's known behaviour. The specific risks: the redirect-return path must still enter directly or the no-popup flow loops; and prompt:'select_account' behaviour under a real IdP is argued from ENCA's precedent here, not executed. Graduates on one manual pass of the flows against the real IdP.",
      test: [
        "THE ONE THAT MATTERS: sign in, press F5. You must land on the sign-in screen with NO resume note, and clicking Sign in must open a REAL prompt \u2014 the account chooser \u2014 not drop you straight into the app. Entering without a prompt is the exact regression this item exists to prevent.",
        "Complete the popup sign-in and confirm you enter; then use the no-popup redirect link end to end \u2014 landing back from the IdP must enter DIRECTLY, no second click, no loop.",
        "Sign out, sign in: chooser again, and picking the other account must land in the other tenant \u2014 the consultancy-laptop case.",
        "Mid-session, open T02 or T06: incremental scopes must still come silently \u2014 adopt() survived for exactly this, and losing it would make every tool prompt.",
        "On the cfdev tenant, confirm the badge appears after the interactive sign-in \u2014 enter() unchanged, only the road to it.",
      ],
    ...
    },
    {
      ...
      test: [
        ...
        "Click Sign out: back to the sign-in screen, box hidden, and the next Sign in must ASK rather than silently continue the session just ended (item 57's always-a-sign-in contract, it shares the handler).",
    ...
    {
      n: 45,
      title: "Silent tokens inside the session, and T01's column order",
      tools: ["All tools", "\ud83d\udd10 AppLocker builder & validator"],
      builds: [10361],
      risk: "medium",
      what: "What survives of 10361 after item 57 (build 10376) restored ENCA's always-interactive sign-in: adopt() sets the MSAL ACTIVE account alongside the local one after an interactive sign-in completes, which acquireTokenSilent in js/graph.js needs or every incremental-consent token re-prompts; sign out clears the active account so the next sign-in cannot silently reuse the one just left. And T01's left column is reordered: enforcement modes and the add-rule form above the evidence, coverage, findings and rule list. 10361's other two changes — the cached-session restore on refresh and the removal of prompt:'select_account' — were REVERSED by item 57; the queue number 53 (10371's restore/gate split, also superseded) is retired.",
      why: "MEDIUM \u2014 down from high: the sign-in entry path itself is item 57's now. What is left here is the silent-token plumbing inside a session (a failure re-prompts, it does not mis-enter) and a layout change. Graduates when an incremental scope (T02 or T06) is acquired silently mid-session after one interactive sign-in.",
      test: [
        "THE ONE THAT MATTERS: sign in, open a tool that needs an incremental scope (T02 or T06). The consent/token step must be silent (or a consent prompt at most on first ever use) \u2014 if it re-runs a full authentication, setActiveAccount is not taking effect.",
        "Sign out, sign in again: the chooser must appear (item 57's contract) and the newly picked account's tokens must acquire silently afterwards.",
        "In T01: confirm the enforcement card and the add-rule form sit above the coverage and findings, adding a rule works from the new position, and the rule list renders below.",
      ],
