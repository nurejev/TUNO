...
const PROMOTE = {
  ...
  items: [
    {
      n: 131,
      title: "T25 🧹 Entra device cleanup — disable at 30, delete at 90 after disable (R36)",
      tools: ["T25"],
      builds: [10532, 10533],
      files: ["js/devicecleanup.js", "js/graph.js", "js/app.js", "index.html", "New-TunoAppRegistration.ps1", "SECURITY.md", "js/version.js", "js/changelog.js", "js/promote.js"],
      risk: "high",
      what: "New tool, Mihai's spec verbatim: stale ENTRA devices (the directory object, /devices, approximateLastSignInDateTime — Device.Read.All held since 10330) staged out in TWO steps — 30 days silent → DISABLE (reversible), disabled AND 90 days silent → DELETE. A 90-day device still enabled is a DISABLE candidate, never a delete: delete follows disable by construction. UNKNOWN IS NOT STALE: a device with no last sign-in at all (a never-booted Autopilot object, an API blank) is its own actionless bucket — acting on it would be a guess. THE WRITES: Device.ReadWrite.All, the registration's FIRST directory-device write, new at this build per the R18 rule (registration script + SECURITY.md in this commit); Graph additionally gates device writes on the signed-in user's DIRECTORY ROLE — the T18 second-gate rule, named before the run, a post-consent 403 translated. T11's pipeline sized for devices: fresh per-device read before every write (signed in since the plan → refused, it woke up; re-enabled → refused, somebody wants it back), read-back after (delete verified by the directory no longer returning the object); deletes arm only after typing DELETE because the BitLocker recovery keys die with the object; Autopilot-registered devices are refused by the service in its own words, reported per row. Thresholds configurable (30/90 defaults); the 10531 three-face selection on both tables; managed (Intune-enrolled) devices wear a chip saying deletion does not unenrol them. 📝 ONE MARKDOWN REPORT from the same state the screen renders: estate, actions with outcomes, everything deliberately not done with its reason, the unknowns. 10533 surfaces the selection controls: Select all/none ABOVE each table (they sat below a 21-row list) with a live ticked count.",
      why: "HIGH, and the highest-risk writer in the app: DELETE on directory objects is not reversible and takes BitLocker recovery keys with it. The containment is layered — two-stage by construction (nothing is deletable until it was already disabled, and disabling is reversible), the typed DELETE, the fresh per-device re-check that refuses awakened or re-enabled devices, the read-back verify, unknown-never-stale, and Graph's own role gate underneath everything. The new scope is the other half of the risk: one more admin-consent round trip per tenant, and a write permission on directory objects in an app that until now could only write Intune configuration — said in SECURITY.md in the same commit. What real eyes must do before promoting: run the full disable→wait→delete cycle against real hardware in the dev tenant, and check the report reads correctly to somebody who was not in the room.",
      test: [
        "A tenant with mixed staleness: the five buckets add up to the device count, and every 90-day-plus ENABLED device sits in disable, not delete.",
        "A device with no approximateLastSignInDateTime: in the unknown bucket, no checkbox anywhere near it.",
        "Disable two ticked devices: both verified by read-back, re-reading re-buckets them into disabled-waiting.",
        "Delete without typing DELETE: the button stays dead. Type it: the button arms.",
        "Re-enable a delete candidate in the portal, then apply its delete: REFUSED as re-enabled — nothing deleted.",
        "Sign a stale device in (or move its clock), then apply: REFUSED as woke-up.",
        "An Autopilot-registered device in the delete set: the service refuses; the row carries Graph's own words.",
        "A signed-in admin WITHOUT a device-capable role: consent succeeds, the write 403s, and the error names the role gate rather than printing the status code.",
        "📝 the report: estate numbers match the cards; every action taken appears with its outcome; the refused/skipped rows carry their reasons; the unknowns section names its rule.",
        "First run on a fresh tenant: the consent prompt shows Device.ReadWrite.All — the R18 cost, visible, once.",
      ],
    },
    {
      n: 130,
