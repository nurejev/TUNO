...
const PROMOTE = {
  ...
  items: [
    ...
    },
    {
      ...
      builds: [10435, 10436, 10437, 10438, 10440, 10441, 10442, 10443, 10445],
      risk: "medium",
      what: "T01 v0.15. The fleet evidence card moved OUTSIDE the policy-only body: an events bundle uploaded first renders immediately (the hunt-for-a-file alert is gone) and the card leads with a chooser — upload the draft, or PULL THE DEPLOYED PROFILE FROM THE TENANT. The pull lists Graph.customProfiles(), keeps only profiles whose omaSettings carry AppLocker OMA-URIs, and rebuilds the policy from their RuleCollection values; the profile's NAME and GROUPING are adopted into the Intune form so a later export edits in place and the collision stop refuses a twin. Values Graph withholds → refusal with a reason, never a half-rebuilt policy. Read-only, existing scope (DeviceManagementConfiguration.ReadWrite.All already covers the read). 10436: the chips are FILTERS (cap applies after the filter; gap report ignores it on purpose), ✕ Clear drops just the evidence, 🧹 Start over empties the table behind a tenant-untouched confirm, and the upload button names all three files. 10437: DLL events are SET ASIDE — out of the default view and the undecided count, own chip with the why per row, no fix button, own gap-report section; they classify normally only when the draft carries DLL rules. 10438: the upload split into two buttons — policy entrance and events entrance — same picker, content-routed as before. 10440: the name\u2019s version token iterates — tenant pull puts V4.0.1 on the table for a deployed V4.0, +v increments by hand, no token = untouched — and the pull now syncs the form inputs it changes. 10441: ONE TABLE — fleet gaps are 📡 findings closed through the standard fix framework; the evidence card is a summary (facts, counts line, gap report, tenant chooser); the 10436 chips retired with the second table. 10442: rules and findings merged into ONE card (mockup round, option A) — rule-attached findings nest under their rule with fix buttons and in-place editors, rule-less ones keep the top table, #alRules is empty, the fix framework and index space untouched. 10443: sticky card header; coverage folded in as a section; the events finding carries its audited list expandable in place; ONE FIX FLOW — fleet and unsigned-hash fixes open the same confirm card as the editors, Apply commits, ✔ Applied + Undo lands in the sticky header; unchanged publisher Apply is refused with the reason; the unsigned finding gained a hash-rules fix card. 10445: the sticky header actually sticks — .list-card clips (overflow:hidden) and sticky never engages inside a clipped ancestor; #alFindings now overflows visible. VERIFY IN A REAL BROWSER: jsdom cannot test sticky.",
      ...
      test: [
        ...
        "10442: on a real policy check the merged card — every rule-attached finding must sit under its rule with a working fix and editor, the compact summary must appear in the narrow column with the rules still reachable below it, and nothing from the old Rules card may be missing (delete buttons included).",
        "10441: upload a real bundle with the draft loaded — every gap must appear as a 📡 finding with a working one-click fix and undo; the evidence card must carry NO second table, and its counts must match the findings and the gap report; Clear must remove the 📡 rows with the evidence and keep the policy.",
      ],
