...
const PROMOTE = {
  ...
  items: [
    {
      n: 188, title: "\ud83d\udd10 T01 \u2014 the device scan as a Remediation (scanner 1.13.0 + Detect-TunoAppLockerScan.ps1, fourth pair) and \ud83d\udcc1 From the harvest site on Evidence (NEW SCOPES: Sites.Read.All on Graph, AllSites.Read on SharePoint Online)",
      tools: ["T01 AppLocker builder & validator"], builds: [10617, 10618, 10619, 10620], risk: "medium",
      ...
      test: [
        "10618-10620 (devcf, 18 Sep): the site read and the listing worked first time; Import failed three times through Graph — no download URL from a $select naming the annotation (10618), Failed to fetch on the pre-authenticated URL because SharePoint's download host answers no CORS (10619), and the $batch hands back that same redirect (10620). Since 10620 the bytes are read from SharePoint's own REST with a token for AllSites.Read on the SharePoint Online API (the registration's ONE non-Graph permission — re-run New-TunoAppRegistration.ps1, which now grants it on the SharePoint service principal, or consent at the click). Re-test: Import newest events bundle asks the SharePoint consent once and lands the bundle on the table; if it still fails the card says what SharePoint REST and Graph each refused, and Open \u2197 is the by-hand route.",
